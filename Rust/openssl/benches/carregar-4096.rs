...
fn benchmark_generation_keys(c: &mut Criterion) {
    ...
    for iteration in [1] {
        group.bench_with_input(
            ...
            &iteration,
            |b, &_iteration| {
                b.iter(|| Rsa::generate(4096).expect("Falha ao gerar a chave RSA"));
            },
        );
    }
    group.finish();
...
fn benchmark_encrypt(c: &mut Criterion) {
    if !std::path::Path::new("public_4096.pem").exists() {
        eprintln!("Chave pública não encontrada, não é possível continuar o benchmark.");
        return;
    }

    let rsa = load_public_key("public_4096.pem");
...
fn benchmark_decrypt(c: &mut Criterion) {
    if !std::path::Path::new("private_4096.pem").exists() {
        eprintln!("Chave privada não encontrada, não é possível continuar o benchmark.");
        return;
    }

    let rsa_private = load_private_key("private_4096.pem");
    let rsa_public = load_public_key("public_4096.pem");
