    ...
    for iteration in [1] {
        group.bench_with_input(
            ...
            |b, &_iteration| {
                b.iter(|| {
                    let priv_key =
                        RsaPrivateKey::new(&mut rng, 4096).expect("Falha ao gerar a chave privada");
...
fn benchmark_encrypt(c: &mut Criterion) {
    ...
    let public_key = load_public_key("public_4096.pem").expect("Falha ao carregar a chave pública");
...
fn benchmark_decrypt(c: &mut Criterion) {
    ...
    let public_key = load_public_key("public_4096.pem").expect("Falha ao carregar a chave pública");
    let private_key = load_private_key("private_4096.pem").expect("Falha ao carregar a chave privada");
